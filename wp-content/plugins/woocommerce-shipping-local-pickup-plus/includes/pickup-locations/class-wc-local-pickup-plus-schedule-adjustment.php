...
class WC_Local_Pickup_Plus_Schedule_Adjustment {
	...
	private function get_relative_datetime( $date = null, $format = 'mysql' ) {
		...
		if ( is_int( $date ) ) {
			...
			if ( 'lead-time' === $this->id && $location = wc_local_pickup_plus_get_pickup_location( $this->location_id ) ) {

				$day_of_week  = date( 'w', $date );
				$start_of_day = strtotime( 'today', $date );
				$schedule     = $location->get_business_hours()->get_value();

				if ( $schedule && ! empty( $schedule[ $day_of_week ] ) ) {

					$closing_time = max( array_values( $schedule[ $day_of_week ] ) );

					$closing = $start_of_day + $closing_time;
