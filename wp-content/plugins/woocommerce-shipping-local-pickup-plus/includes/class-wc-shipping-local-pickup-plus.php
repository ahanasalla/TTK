...
class WC_Shipping_Local_Pickup_Plus extends WC_Shipping_Method {
	...
	protected function get_settings_fields() {
		...
		$form_fields = array(
			...
			'default_handling' => array(
				...
				'desc_tip' => __( 'Choose whether cart items will be set as shipped or for pickup when customers first arrive at the cart or checkout page.', 'woocommerce-shipping-local-pickup-plus' ), // TODO: not sure I like this {CW 2017-06-14}
				'type'     => 'select',
				'options'  => array(
	 ...
	 * @since 2.1.0
	 *
	 * @return string $default_handling either 'pickup' or 'ship'
	 */
	public function get_default_handling() {
		 ...
		 */
		return apply_filters( "woocommerce_shipping_{$this->id}_default_handling", $this->get_option( 'default_handling' ) );
	}
	...
	public function set_default_shipping_method( $shipping_method, $shipping_rates ) {
		return 'pickup' === $this->get_default_handling() && array_key_exists( $this->id, $shipping_rates ) ? $this->id : $shipping_method;
	}
	...
	public function calculate_shipping( $package = array() ) {
		...
		if ( is_cart() || is_checkout() || ( $wp_query && defined( 'WC_DOING_AJAX' ) && 'update_order_review' === $wp_query->get( 'wc-ajax' ) ) ) {
