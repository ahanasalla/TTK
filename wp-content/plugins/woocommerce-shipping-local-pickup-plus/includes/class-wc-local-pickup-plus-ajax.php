		...
		if ( ! empty( $_REQUEST['term'] ) ) {
			...
			// search by address
				...
				$origin = new WC_Local_Pickup_Plus_Address( array(
					...
					'city'     => $search_term, // -> they might be searching by city/town
					'postcode' => $search_term, // -> or they might be searching by postcode
				) );
			}
	...
	public function get_pickup_location_appointment_data() {
		...
		if (    isset( $_POST['location'] )
			...
			if ( $location && 'publish' === $location->get_post()->post_status ) {
				...
				$start_date = $location->get_pickup_lead_time()->get_relative_time( $now );
				$end_date   = $location->get_pickup_deadline()->get_relative_time( $now );
				...
				wp_send_json_success( array(
					'address'           => $address,
					'calendar_start'    => date( 'c', $start_date ),
					'calendar_end'      => date( 'c', $end_date ),
					'unavailable_dates' => $location->get_public_holidays()->get_unavailable_dates( 'Y-m-d', $start_date, $end_date ),
				)  );
			}
		}
