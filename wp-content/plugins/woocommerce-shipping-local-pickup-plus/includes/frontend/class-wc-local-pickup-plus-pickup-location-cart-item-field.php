	...
	public function get_html() {
		...
		if ( $local_pickup_plus->is_available() ) {
			...
			if ( $product && wc_local_pickup_plus_product_can_be_picked_up( $product ) ) {
				...
				// sanity check
				if ( $local_pickup_plus->is_per_item_selection_enabled() && ! wc_local_pickup_plus_product_can_be_picked_up( $product, $chosen_pickup_location ) ) {
				...
				<div
					...
					if ( $local_pickup_plus->is_per_item_selection_enabled() ) : ?>
						...
						<div style="display: <?php echo $must_be_picked_up || $should_be_picked_up ? 'block' : 'none'; ?>;">
							<?php echo $this->get_location_select_html( $cart_item_id, $chosen_pickup_location ); ?>
						</div>

					<?php endif; ?>

					<?php if ( ! $must_be_picked_up ) : ?>
						<small style="display: <?php echo   $should_be_picked_up ? 'none' : 'block'; ?>;"><a class="enable-local-pickup"  href="#"><?php esc_html_e( 'I want to pickup this item',     'woocommerce-shipping-local-pickup-plus' ); ?> &raquo;</a></small>
						<small style="display: <?php echo ! $should_be_picked_up ? 'none' : 'block'; ?>;"><a class="disable-local-pickup" href="#"><?php esc_html_e( 'I want this item to be shipped', 'woocommerce-shipping-local-pickup-plus' ); ?> &raquo;</a></small>
					<?php endif; ?>
