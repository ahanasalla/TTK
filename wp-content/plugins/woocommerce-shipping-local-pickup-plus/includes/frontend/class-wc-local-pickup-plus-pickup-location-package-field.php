...
class WC_Local_Pickup_Plus_Pickup_Location_Package_Field extends WC_Local_Pickup_Plus_Pickup_Location_Field {
	...
	private function get_pickup_date() {
		return ! $this->pickup_location_has_changed() ? $this->get_pickup_data( 'pickup_date' ) : '';
	}
	...
	protected function get_pickup_location_html() {
		...
		if ( ! wc_local_pickup_plus_shipping_method()->is_per_item_selection_enabled() ) :
			...
			echo $this->get_location_select_html( $this->get_package_id(), $chosen_location );
	...
	public function get_html() {
		...
		if ( $shipping_method && $shipping_method->is_available() ) {
			...
			<div
				...
				echo $this->get_pickup_location_html(); ?>

				<?php // display the pickup appointment fields at checkout if enabled
				if ( is_checkout() && 'disabled' !== $shipping_method->pickup_appointments_mode() && $this->get_pickup_location() ) : ?>
