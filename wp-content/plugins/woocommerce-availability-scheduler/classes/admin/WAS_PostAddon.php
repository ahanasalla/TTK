...
/*  public static function get_post_expiring_date($post_id)
 {
	 return get_post_meta( $post_id, '_was_expiring_date', true);
 } */
 public static function get_post_meta($post_id)
 {
	global $was_product_model;
	return $was_product_model->get_product_meta($post_id);
 }
 ...
 function render_scheduler_availability_box($post) 
	{ 
		...
		wp_enqueue_script('was-admin-produt-detail-page', AS_PLUGIN_PATH.'/js/was-admin-produt-detail-page.js', array('jquery')); 		
		...
		$days = get_post_meta( $post->ID, '_was_period', true);
		$start_times = get_post_meta( $post->ID, '_was_start_time', true);
		$end_times = get_post_meta( $post->ID, '_was_end_time', true);
		$product_page_msg = get_post_meta( $post->ID, '_was_product_page_msg', true);
		$product_page_during_msg = get_post_meta( $post->ID, '_was_product_page_during_msg', true);
		$product_page_after_msg = get_post_meta( $post->ID, '_was_product_page_after_msg', true);
		$shop_page_msg = get_post_meta( $post->ID, '_was_shop_page_msg', true);
		$shop_page_during_msg = get_post_meta( $post->ID, '_was_shop_page_during_msg', true);
		$shop_page_after_msg = get_post_meta( $post->ID, '_was_shop_page_after_msg', true);
		$countdown_to_start = get_post_meta( $post->ID, '_was_countdown_to_start', true);
		$was_hide = get_post_meta( $post->ID, '_was_hide', true);
		$was_total_sales_limit = get_post_meta( $post->ID, '_was_total_sales_limit', true);
		$countdown_to_end = get_post_meta( $post->ID, '_was_countdown_to_end', true);
		$where_countdown_to_start = get_post_meta( $post->ID, '_was_where_countdown_to_start', true);
		$where_countdown_to_end = get_post_meta( $post->ID, '_was_where_countdown_to_end', true);
		$where_sales_progressbar = get_post_meta( $post->ID, '_was_where_sales_progressbar', true);
		$expiring_date = get_post_meta( $post->ID, '_was_expiring_date', true);
		$visible_after_expiring_date = get_post_meta( $post->ID, '_was_visible_after_expiring_date', true);
		$allowed_roles = get_post_meta( $post->ID, '_was_customer_roles', true);
		$customers_ids = get_post_meta( $post->ID, '_was_customer_ids', true);
		$customer_roles_restriction_type = get_post_meta( $post->ID, '_was_customer_roles_restriction_type', true);
		$customer_roles_restriction_message = get_post_meta( $post->ID, '_was_customer_roles_restriction_message', true);
		...
		<div id="wcas_tabs">
			...
			<?php for ($day=0; $day<7; $day++): ?>
				<div id="tabs-<?php echo $day;?>">
						...
						<!--	<option value="no_restriction" <?php if($customer_roles_restriction_type[$day] == "no_restriction") echo 'selected="selected"'; ?>><?php  _e('No restriction', 'woocommerce-customers-manager');?></option> -->
							<option value="apply_to_selected" <?php if($customer_roles_restriction_type[$day] == "apply_to_selected") echo 'selected="selected"'; ?>><?php  _e('Apply to selected roles', 'woocommerce-customers-manager');?></option>
							<option value="except_the_selected" <?php if($customer_roles_restriction_type[$day] == "except_the_selected") echo 'selected="selected"'; ?>><?php  _e('Apply to all except the selected roles', 'woocommerce-customers-manager');?></option>
						</select>
