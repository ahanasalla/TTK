		...
		<table class="wccpf_fields_table <?php echo apply_filters( 'wccpf/fields/container/class', '' ); ?><?php echo $field_class.'-wrapper'; ?>" cellspacing="0">
			<tbody>
				<tr>
					...
					<p><?php echo esc_attr( $field["default_value"] ); ?></p>
					<?php else: ?>
						<input type="text" readonly class="wccpf-field<?php echo $field_class; ?>" name="<?php echo esc_attr( $field["name"] . $name_index ); ?>" value="<?php session_start(); echo $_SESSION['category_date']; ?>" placeholder="<?php echo esc_attr( $field["placeholder"] ); ?>" maxlength="<?php echo esc_attr( $field["maxlength"] ); ?>" wccpf-type="text" wccpf-pattern="mandatory" wccpf-mandatory="<?php echo $field["required"]; ?>" <?php echo $readonly; ?> />
						<span class="wccpf-validation-message wccpf-is-valid-<?php echo $this->valid; ?>"><?php echo $field["message"]; ?></span>
					<?php endif; ?>
					</td>
				</tr>
